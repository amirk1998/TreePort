...
pub fn render(cfg: &Config, stats: &Stats, entries: &[Entry], duplicates: Option<&DuplicateResult>, elapsed: Duration) -> String {
    ...
    for (i, (cat, count, size)) in cats.iter().enumerate() {
        out.push_str(&format!("    {{ \"category\": {}, \"count\": {count}, \"total_size_bytes\": {size} }}{}\n", json_str(cat), if i + 1 < cats.len() { "," } else { "" }));
    }
    out.push_str("  ],\n");
    ...
    for (i, (name, (count, size))) in ext.iter().enumerate() {
        out.push_str(&format!("    {{ \"extension\": {}, \"count\": {count}, \"total_size_bytes\": {size} }}{}\n", json_str(name), if i + 1 < ext.len() { "," } else { "" }));
    }
    out.push_str("  ],\n");

    out.push_str("  \"entries\": [\n");
    for (i, e) in entries.iter().enumerate() {
        let kind = match e.kind { Kind::File => "file", Kind::Dir => "dir", Kind::Symlink => "symlink", Kind::Other => "other" };
    ...
    if let Some(result) = duplicates {
        out.push_str(",\n  \"duplicates\": {\n");
        out.push_str(&format!("    \"skipped_too_large\": {},\n", result.skipped_too_large));
        out.push_str("    \"groups\": [\n");
        for (gi, group) in result.groups.iter().enumerate() {
            ...
            for (fi, entry) in relative_paths(group, entries).enumerate() {
                ...
                if fi + 1 < group.len() { out.push_str(", "); }
            }
            out.push_str(&format!("] }}{}\n", if gi + 1 < result.groups.len() { "," } else { "" }));
        }
        out.push_str("    ]\n  }\n");
    } else {
        out.push('\n');
...
fn json_escape(s: &str) -> String {
    ...
    for c in s.chars() {
        match c { '"' => out.push_str("\\\""), '\\' => out.push_str("\\\\"), '\n' => out.push_str("\\n"), '\r' => out.push_str("\\r"), '\t' => out.push_str("\\t"), c if (c as u32) < 0x20 => out.push_str(&format!("\\u{:04x}", c as u32)), c => out.push(c) }
    }
    out
}
