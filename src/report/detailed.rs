...
pub fn render(root: &Path, entries: &[Entry]) -> String {
    ...
    for entry in entries {
        ...
        let size = if entry.kind == crate::model::Kind::File { human_size(entry.size) } else { "-".to_string() };
        #[cfg(unix)]
        let perms = crate::util::format_mode(entry.mode);
        #[cfg(not(unix))]
        let perms = "n/a".to_string();
        out.push_str(&format!("{:<5} {:>12} {:<20} {:<9} {}{}\n", entry.kind, size, format_time(entry.modified), perms, indent, bidi_safe(&name)));
    }
    out
}
