...
pub fn render(
    ...
    for (i, file) in files.iter().take(cfg.top_n).enumerate() {
        out.push_str(&format!(
            "| {} | {} | `{}` |\n",
            i + 1,
            human_size(file.size),
            md_escape(&rel(&file.path))
        ));
    }
    ...
    if let Some(result) = duplicates {
        ...
        if result.skipped_too_large > 0 {
            out.push_str(&format!("*Note: {} file(s) were skipped from duplicate checking for being larger than {}.*\n\n", result.skipped_too_large, human_size(DUP_HASH_SIZE_CAP)));
        }
    }
