...
pub fn render(root: &Path, entries: &[Entry]) -> String {
    ...
    for entry in entries {
        ...
        let kind = match entry.kind { Kind::File => "file", Kind::Dir => "dir", Kind::Symlink => "symlink", Kind::Other => "other" };
        out.push_str(&format!("{},{},{},{},{},{}\n", csv_escape(&rel), kind, entry.size, entry.depth, csv_escape(&crate::util::format_time(entry.modified)), entry.hidden));
    }
    out
}
