...
pub fn render(root: &Path, entries: &[Entry]) -> String {
    ...
    for (i, entry) in files.into_iter().enumerate() {
        let name = entry.path.file_name().map(|n| n.to_string_lossy().into_owned()).unwrap_or_else(|| entry.path.display().to_string());
        let rel = relative(root, &entry.path);
        out.push_str(&format!("{},{},{}\r\n", i + 1, csv_escape(&name), csv_escape(&rel)));
    }
    out
}
...
pub fn csv_escape(s: &str) -> String {
    if s.contains(',') || s.contains('"') || s.contains('\n') || s.contains('\r') { format!("\"{}\"", s.replace('"', "\"\"")) } else { s.to_string() }
}
...
mod tests {
    ...
    fn entry(path: &str, kind: Kind) -> Entry {
        Entry { path: PathBuf::from(path), kind, size: 1, depth: 1, modified: None, hidden: false, #[cfg(unix)] mode: None }
    }

    #[test]
    fn numbered_sorted_utf8_csv() {
        let root = Path::new("/proj");
        let entries = vec![entry("/proj/گزارش.md", Kind::File), entry("/proj/b.md", Kind::File), entry("/proj/sub", Kind::Dir), entry("/proj/a, \"quoted\".md", Kind::File)];
        let csv = render(root, &entries);
        let rows: Vec<&str> = csv.split("\r\n").collect();
        assert_eq!(rows[0], "File Number,File Name,Relative Path");
        assert_eq!(rows[1], "1,\"a, \"\"quoted\"\".md\",\"a, \"\"quoted\"\".md\"");
    ...
    fn tie_breaks_on_relative_path() {
        let root = Path::new("/proj");
        let entries = vec![entry("/proj/z/notes.md", Kind::File), entry("/proj/a/notes.md", Kind::File)];
        let csv = render(root, &entries);
        let rows: Vec<&str> = csv.split("\r\n").collect();
