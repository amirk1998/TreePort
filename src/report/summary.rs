...
use crate::duplicate::{DUP_HASH_SIZE_CAP, DuplicateResult, relative_paths, wasted_space};
use crate::model::{Entry, Kind, Stats};
use crate::util::{bidi_safe, format_time, human_size, relative_display};

pub fn render_detailed(root: &Path, entries: &[Entry]) -> String {
    super::detailed::render(root, entries)
}
...
pub fn render_summary(
    ...
    for (name, (count, size)) in ext.iter().take(15) {
        out.push_str(&format!(
            "{:<18} {:>10} {:>15}\n",
            name,
            count,
    ...
    for file in files.iter().take(top_n) {
        out.push_str(&format!(
            ...
            bidi_safe(&relative_display(root, &file.path))
        ));
    }
    ...
    if !dirs.is_empty() {
        ...
        for (path, size, count) in dirs {
            out.push_str(&format!(
                "{:>12}  {}  ({count} items)\n",
                human_size(*size),
                bidi_safe(&relative_display(root, path))
            ));
        }
    }

    let mut timeline: Vec<&Entry> = entries
        .iter()
    ...
    if let Some(oldest) = timeline.first() {
        out.push_str(&format!(
            "\nOldest file: {} ({})\n",
            bidi_safe(&relative_display(root, &oldest.path)),
    ...
    if let Some(newest) = timeline.last() {
        out.push_str(&format!(
            "Newest file: {} ({})\n",
            bidi_safe(&relative_display(root, &newest.path)),
        ...
        if result.groups.is_empty() {
            ...
            for (i, group) in result.groups.iter().take(10).enumerate() {
                out.push_str(&format!(
                    "  Group {}: {} x{} copies\n",
                    i + 1,
                    human_size(entries[group[0]].size),
                    group.len()
                ));
                for entry in relative_paths(group, entries) {
                    out.push_str(&format!(
                        "    - {}\n",
